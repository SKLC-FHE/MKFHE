...
#ifndef _MNTRU_CRYPTOPARAMETERS_H_
#define _MNTRU_CRYPTOPARAMETERS_H_
...
/**
 * @brief Class that stores all parameters for the MNTRU scheme
 */
class MNTRUCryptoParams : public Serializable {
private:
    uint32_t m_k;
    // modulus for the additive MNTRU scheme
    NativeInteger m_q{};
    // modulus for the RingGSW/RingMNTRU scheme
    NativeInteger m_Q{};
    // modulus for key-switching
    NativeInteger m_qKS{};
    // lattice parameter for the additive MNTRU scheme
    uint32_t m_n{};
    // ring dimension for RingGSW/RingMNTRU scheme
    uint32_t m_N{};
...
public:
    ...
    MNTRUCryptoParams() = default;

    /**
   * Main constructor for MNTRUCryptoParams
   *
   * @param n lattice parameter for additive MNTRU scheme
   * @param N ring dimension for RingGSW/RMNTRU used in bootstrapping
   * @param q modulus for additive MNTRU
   * @param Q modulus for RingGSW/RMNTRU used in bootstrapping
   ...
   */
        ...
        : m_k(k), m_q(q), m_Q(Q), m_qKS(q_KS), m_n(n), m_N(N), m_baseKS(baseKS), m_keyDist(keyDist) 
    {
        ...
        if(!m_q)
            OPENFHE_THROW(config_error, "m_q (modulus for additive MNTRU) can not be zero");
        if(!m_Q)
            OPENFHE_THROW(config_error, "m_Q (modulus for RingGSW/RMNTRU) can not be zero");
        if(!q_KS)
    ...
    MNTRUCryptoParams(const MNTRUCryptoParams& rhs)
        : m_k(rhs.m_k),
          m_q(rhs.m_q),
          m_Q(rhs.m_Q),
          // m_qKS(rhs.m_qKS),
    ...
    MNTRUCryptoParams(MNTRUCryptoParams&& rhs) noexcept
    ...
    MNTRUCryptoParams& operator=(const MNTRUCryptoParams& rhs) {
        this->m_k = rhs.m_k;
        this->m_q = rhs.m_q;
        this->m_Q = rhs.m_Q;
    ...
    MNTRUCryptoParams& operator=(MNTRUCryptoParams&& rhs) noexcept {
    ...
    bool operator==(const MNTRUCryptoParams& other) const {
...
#endif  // _MNTRU_CRYPTOPARAMETERS_H_
